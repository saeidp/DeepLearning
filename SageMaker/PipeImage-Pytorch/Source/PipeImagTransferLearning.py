...
def transfer_learning(args):
    ...
    for epoch in range(1, args.epochs + 1):     
        ...
        # log Confusion matrix for epoch
        logger.info('Confusion matrix: \n{}'.format(conf_matrix.value()))
        logger.info('\n\n')
...
def validate(model, valid_loader, NUM_CLASSES,batch_size, device):
    ...
    return epoch_loss,epoch_acc,conf_matrix,wrong_pred_paths
...
if __name__ == '__main__':
    ...
    model_meta_data=os.path.join(os.environ['SM_MODEL_DIR'], "modelMetaData.txt")
   
    #Creating a list of images used for training,validation and testing
    train_path=os.path.join(os.environ['SM_MODEL_DIR'], "TrainImages.txt")
    
    logger.info("Saving the training images to {}.".format(train_path))
    train_text_file = open(train_path, "w")
    train_text_file.write("Train Images:")
    train_text_file.write('\r\n')
    
    for dirpath, dirnames, filenames in os.walk(env.channel_input_dirs.get('training')+'/train'):
        train_images_list=[f for f in filenames if f.endswith((".png",".jpg"))]
        #logger.info(train_images_list)
    ...
    logger.info('Folders name:')
    logger.info(folders)
    classes.sort()
                            
    events={v: k for v, k in enumerate(classes)}
                            
    model_data['Classes']=len(classes)
    model_data['Labels']=events
    
    logger.info('Events mapping: ')
    logger.info(events)
    
    with open(model_meta_data, 'w') as outfile:  
        json.dump(model_data, outfile)
                            
    test_path=os.path.join(os.environ['SM_MODEL_DIR'], "TestImages.txt")
    logger.info("Saving the Test images to {}.".format(test_path))
    test_text_file = open(test_path, "w")
    test_text_file.write("Test Images:")
    test_text_file.write('\r\n')
    for dirpath, dirnames, filenames in os.walk(env.channel_input_dirs.get('training')+'/test'):
            dirct=dirpath.split('/')
            for filename in [f for f in filenames if f.endswith((".png",".jpg"))]:
                if len(dirct)>6:
                    prefix=('/').join(dirct[-2:])
    ...
    val_path=os.path.join(os.environ['SM_MODEL_DIR'], "ValidationImages.txt")
    logger.info("Saving the validation images to {}.".format(val_path))
    val_text_file = open(val_path, "w")
    val_text_file.write("Validation Images:")
    val_text_file.write('\r\n')
    for dirpath, dirnames, filenames in os.walk(env.channel_input_dirs.get('training')+'/validation'):
            dirct=dirpath.split('/')                    
            for filename in [f for f in filenames if f.endswith((".png",".jpg"))]:
                if len(dirct)>6:
                    prefix=('/').join(dirct[-2:])                
                    val_text_file.write(os.path.join(args.data_location,'validation', filename))
                val_text_file.write('\r\n')
    ...
    #logger.info(('/').join(model_artifacts))
    ...
    #logger.info(model_dir_location)
    
    if args.model_location!=None:
        model_artifacts=args.model_location.split('/')
        my_bucket = resource.Bucket(model_artifacts[2]) 
        model_path=('/').join(model_artifacts[3:])
        output_model_path=model_artifacts[-1]
